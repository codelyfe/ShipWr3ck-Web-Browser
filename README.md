...
Welcome to ShipWr3ck Web Browser, a powerful privacy-focused browser with advanced customization and built-in tools. - TEASER: https://www.youtube.com/watch?v=Yb7EscDnlvE
...
```
ShipWr3ck/
├── main.js          # Electron main process (BrowserWindow setup)
├── index.html        # Browser UI structure
├── renderer.js       # Frontend logic and IPC communication
├── styles.css        # Modern dark theme styling
├── tor-config.js     # Tor proxy configuration module
├── EditWhat.html     # Advanced text editor (C0D3LYF3) with 100+ commands
├── terminal.html     # Logical Terminal with 50+ commands
├── myrepos.html      # Code Bank - GitHub-like repository browser
├── dashboard.html     # Dashboard with speed dial and network activity
├── readme.html       # Complete user guide (HTML version)
...
- **main.js:** Creates the main Electron window and a BrowserView for rendering web content
- **BrowserView:** Handles all web page rendering, isolated from the UI
- **IPC Communication:** Enables communication between the UI (renderer) and main process
- **Smart URL Formatting:** Automatically adds `https://` to URLs or converts queries to Google searches

## Building for Distribution

To package the app for distribution, you can use tools like:
- `electron-builder`
- `electron-packager`
...
ShipWr3ck now includes a portable target for Windows so you can run the browser without installing it system-wide.

```bash
# Generate a portable x64/ia32 executable in the dist folder
npm run build:portable
```

This produces a self-contained `.exe` that unpacks to a temporary directory on launch. Copy the generated artifact to a USB drive or any folder that needs a portable browser.
